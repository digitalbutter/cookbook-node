...
Include the node-js recipe to download, compile and install node along with npm.

== defines:

+node+ provider to start/stop server. The following example will start a provided script with node. Logs can be found in /var/log/node-my_node_server.log.

    node_js "my_node_server" do
...
+node_js_npm+ provider to manage nodejs modules. The following example will install the connect module for nodejs.

    node_js_npm "connect" do
        action :install
    end
